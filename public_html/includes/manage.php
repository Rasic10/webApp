...
class Manage
{
	...
	public function manageSearch($table,$pno,$txt){
		$a = $this->paginationForSearch($this->con,$table,$pno,4,$txt);
		if ($table == "categories") {
			...
			$sql = "SELECT p.pid,p.product_name,c.category_name,b.brand_name,p.product_price,p.product_stock,p.added_date,p.p_status FROM products p,brands b,categories c WHERE p.bid = b.bid AND p.cid = c.cid AND p.product_name LIKE '%".$txt."%'".$a["limit"];
		}else{
	...
	private function paginationForSearch($con,$table,$pno,$n,$txt){
		$query = $con->query("SELECT COUNT(*) r FROM products WHERE product_name LIKE '%".$txt."%'");
		$row = mysqli_fetch_assoc($query);
		//$totalRecords = 100000;
		$pageno = $pno;
		$numberOfRecordsPerPage = $n;
